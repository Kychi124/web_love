...
<div class="d-flex justify-content-between align-items-center px-5 my-4 fs-5">
    <div>
        ...
        >
        <a href="categorie.php" class="text-black">categories management</a>
    </div>
    <button 
        class="btn btn-success p-3 mx-4"
        onclick="window.location.href='categorie_add.php'">
        Add Category
    </button>
</div>
...
?>
        <tbody>
            ...
            <td>
                <button class="btn btn-warning" onclick="window.location.href='categorie_edit.php?id=<?php echo $row['category_id'];?>'">แก้ไข</button>
                <button class="btn btn-danger" onclick="window.location.href='categorie_delete.php?id=<?php echo $row['category_id'];?>'">ลบ</button>
            </td>
            </tr>
<?php 
        }while($row = mysqli_fetch_assoc($result));
    }else{
        echo "<tr><td colspan ='8'>ไม่มีข้อมูลประเภทสินค้า</td></tr>";
    }
    ?>
        </tbody>
        </table>
